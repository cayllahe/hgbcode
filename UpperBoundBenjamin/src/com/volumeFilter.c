...
double * computeNodeVolume(JCctree *CT, double * level, int * area)
{
	double * volume = myCalloc(CT->nbnodes, sizeof(double));
	int i;
	JCctreenode *tabnodes = CT->tabnodes;
	for(i=0;i<CT->nbnodes;++i)
	{
		if(i<=CT->nbnodes/2)
		{
			...
			{
				int child = soncell->son;
				volume[i]+=volume[child];	
			}
		}
		int p = tabnodes[i].father;
		if(p>=0)
		{
			double diff = fabs(level[p]-level[i]);
			volume[i]+=area[i]*diff;
		}
	}
...
int32_t main(int32_t argc, char ** argv) 
{
	...
	char * stop;
	double thrp = strtod(argv[2], &stop);
	if(stop[0]!='\0' || thrp <=0 || thrp >=1)
	{
		fprintf(stderr,"%s error: second argument must be in ]0,1[ %s\n", argv[0], argv[4]);
		exit(1);
	}
	...
	double * nodeVolume = computeNodeVolume(CT, level, nodeArea);
	int nbLeaves = CT->nbnodes/2+1;
	JCctreenode *tabnodes = CT->tabnodes;
   	
	double thr=(thrp/100.0)*nodeVolume[CT->root];
    for(i=nbLeaves ; i<CT->nbnodes; ++i)
    {
        JCsoncell * soncell = tabnodes[i].sonlist;
		int child1 = soncell->son;
		int child2 = soncell->next->son;
        if(nodeVolume[i] < thr || nodeVolume[child1] < thr || nodeVolume[child2] < thr)
            level[i]=0;
    }
